 ...
 * that use dot-notation to select a service endpoint.  For example:
 * <pre>
 * {
 *    "jsonrpc": "2.0",
 ...
 * </code>
 * A client can then call a <i>base(String, String)</i> method on the Foo service
 * like this:
 * <pre>
 * {
 *    "jsonrpc": "2.0",
...
public class JsonRpcMultiServer extends JsonRpcServer {
	...
	private final ConcurrentHashMap<String, Class<?>> interfaceMap;
	private char separator = DEFAULT_SEPARATOR;
	
	public JsonRpcMultiServer() {
		this(new ObjectMapper());
		logger.debug("created empty multi service");
	}
	...
	protected Object getHandler(String serviceName) {
		JsonrpcHandlerInvoker handler =getInvokerHandler(serviceName);
		if (handler == null) {
			logger.error("Service '{}' is not registered in this multi-service", serviceName);
	...
	protected JsonrpcHandlerInvoker getInvokerHandler(String serviceName) {
		JsonrpcHandlerInvoker handler = (JsonrpcHandlerInvoker) handlerMap.get(serviceName);
		if (handler == null) {
			logger.error("Service '{}' is not registered in this multi-service", serviceName);
			throw new RuntimeException("Service '" + serviceName + "' does not exist");
		}
