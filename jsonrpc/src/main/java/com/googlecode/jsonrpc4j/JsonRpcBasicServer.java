...
import com.xw.exception.SessionCheckFailException;
...
/**
 * A JSON-RPC request service reads JSON-RPC requests from an input stream and writes responses to an output stream.
...
public class JsonRpcBasicServer {
	...
	private static ConcurrentHashMap<Method,ParameterCount> methodCache = new ConcurrentHashMap<>(2000);
	...
	/**
	 * Creates the service with the given {@link ObjectMapper} delegating
	...
	public JsonRpcBasicServer(final ObjectMapper mapper, final Object handler, final Class<?> remoteInterface) {
		...
		if (handler != null)
			logger.debug("created service for interface {} with handler {}", remoteInterface, handler.getClass());
	}
	
	/**
	 * Creates the service with a default {@link ObjectMapper} delegating
	 * all calls to the given {@code handler} {@link Object} but only
	 * methods available on the {@code remoteInterface}.
	 *
	 * @param handler         the {@code handler}
	 ...
	 * @throws IOException on error
	 */
	public int handleRequest(final InputStream input, final OutputStream output) throws IOException, NoSuchMethodException {
	 ...
	 * @throws IOException on error
	 */
	protected JsonError handleJsonNodeRequest(final JsonNode node, final OutputStream output) throws IOException, NoSuchMethodException {
	 ...
	 * @throws IOException on error
	 */
	private JsonError handleArray(ArrayNode node, OutputStream output) throws IOException, NoSuchMethodException {
	 ...
	 * @throws IOException on error
	 */
	private JsonError handleObject(final ObjectNode node, final OutputStream output) throws IOException, NoSuchMethodException {
		...
		AMethodWithItsArgs methodArgs = null;
		try {
			methodArgs = findBestMethodByParamsNode(methods, node.get(PARAMS),getInvokerHandler(serviceName),partialMethodName,fullMethodName);
		}catch (Exception e){
			shouldLogInvocationErrors = false;
			methodArgs = new AMethodWithItsArgs(JsonRpcBasicServer.class.getDeclaredMethod("handleObject",new Class<?>[]{ObjectNode.class,OutputStream.class}), node);
	...
	protected JsonrpcHandlerInvoker getInvokerHandler(String serviceName) {
		return null;
	}
	 ...
	 */
	private AMethodWithItsArgs findBestMethodByParamsNode(Set<Method> methods, JsonNode paramsNode, JsonrpcHandlerInvoker invoker,String methodName,String fullMethodName) throws Exception {
		if (hasNoParameters(paramsNode)) {
			return findBestMethodUsingParamIndexes(methods, 0, null);
		}
		if (invoker.isNeedCheck(methodName) && !checkIsLogin(paramsNode,fullMethodName)){
			throw new SessionCheckFailException("sessionId 验证失败");
		}
	 ...
	 * @return
     */
	private boolean checkIsLogin(JsonNode paramsNode,String fullMethodName) throws Exception {
		String sessionId = buildSessionId(paramsNode);
		if (ApplicationContextUtils.applicationContext != null) {
			String[] loginVerifiaactions =ApplicationContextUtils.applicationContext.getBeanNamesForType(PermissionAuthFilter.class, true, false);
			if (loginVerifiaactions == null || loginVerifiaactions.length == 0) {
				logger.warn("not config PermissionAuthFilter");
				return false;
			}
			if (loginVerifiaactions != null && loginVerifiaactions.length > 1) {
				throw new RuntimeException("Multi PermissionAuthFilter");
			}
			String beanName = loginVerifiaactions[0];
			if (beanName != null && !"".equals(beanName) && ApplicationContextUtils.applicationContext .isTypeMatch(beanName, PermissionAuthFilter.class)) {
				PermissionAuthFilter instance = ApplicationContextUtils.applicationContext.getBean(beanName, PermissionAuthFilter.class);
				return instance.checkPower(sessionId,fullMethodName);
			}
		}
		return  false;
	}

	private String buildSessionId(JsonNode paramsNode) {
		Set<String> fieldNames = collectFieldNames(paramsNode);
		ObjectNode paramNodes = ObjectNode.class.cast(paramsNode);
		String sessionIdFieldName = getSessionName(fieldNames);
		if (sessionIdFieldName != null){
			JsonNode jsonNode = paramsNode.get(sessionIdFieldName);
			String sessionId = jsonNode.asText();
			paramNodes.remove(sessionIdFieldName);
			RequestContextUtils.setSessionId(sessionId);
			return sessionId;
		}
		return null;
	}

	private String getSessionName(Set<String> fieldNames) {
		String sesionName = null;
		if (fieldNames.contains(Constant.SESSIONID))
			sesionName = Constant.SESSIONID;
		else if (fieldNames.contains(Constant.SESSION))
			sesionName = Constant.SESSION;
		else if(fieldNames.contains(Constant.SESSION_ID))
			sesionName = Constant.SESSION_ID;
		return sesionName;
	}
	...
	private AMethodWithItsArgs findBestMethodUsingParamNames(Set<Method> methods, Set<String> paramNames, ObjectNode paramNodes) {
		...
		for (Method method : methods) {
			...
			ParameterCount parStat = methodCache.get(method);
			if (parStat == null) {
				parStat = new ParameterCount(paramNames, paramNodes, parameterTypes, method);
				methodCache.put(method,parStat);
			}
		...
		return new AMethodWithItsArgs(max.method, paramNames, max.allNames, paramNodes,max.allMethodVariables);
	...
	/**
	 * Sets whether or not the service should be backwards
	...
	/**
	 * Sets whether or not the service should re-throw exceptions.
	 *
	...
	/**
	 * Sets whether or not the service should allow superfluous
	...
	/**
	 * Sets whether or not the service should allow less parameters
	...
	private static class AMethodWithItsArgs {
		...
		public AMethodWithItsArgs(Method method, Set<String> paramNames, List<JsonRpcParam> allNames, ObjectNode paramNodes,List<String> allMethodVariables) {
			this(method);
			collectArgumentsBasedOnName(method, paramNames, allNames, paramNodes,allMethodVariables);
		}
		...
		private void collectArgumentsBasedOnName(Method method, Set<String> paramNames, List<JsonRpcParam> allNames, ObjectNode paramNodes,List<String> allMethodVariables) {
			Class<?>[] types = method.getParameterTypes();
			int numParameters = types.length;
			for (int i = 0; i < numParameters; i++) {
				String variableName = allMethodVariables.get(i);
				if (variableName != null && paramNames.contains(variableName)) {
					if (types[i].isArray() && method.isVarArgs() && numParameters == 1) {
						collectVarargsFromNode(paramNodes.get(variableName));
					} else {
						addArgument(paramNodes.get(variableName));
					}
				} else {
					addArgument(NullNode.getInstance());
				}
	...
	private class ParameterCount {
		...
		private final List<String> allMethodVariables ;
		private final Method method;
		...
		public ParameterCount(Set<String> paramNames, ObjectNode paramNodes, List<Class<?>> parameterTypes, Method method) {
			this.allNames = new ArrayList<>();//getAnnotatedParameterNames(method);
			this.method = method;
			int typeCount = 0;
			int nameCount = 0;
			int at = 0;
...
//				at += 1;
//			}
			this.allMethodVariables = new ArrayList<>();
			String[] methodVariableNames =pnd.getParameterNames(method);
			for (String paramName:methodVariableNames){
				boolean hasParamName = paramNames.contains(paramName);
				if (hasParamName) nameCount += 1;
				if (hasParamName && isMatchingType(paramNodes.get(paramName), parameterTypes.get(at))) typeCount += 1;
				at += 1;
				this.allMethodVariables.add(paramName);
			}
		...
		public List<String> getAllMethodVariables() {
			return allMethodVariables;
		}
